...
export default function AdminArticlesPage() {
  ...
  async function onSave(e: FormEvent) {
    e.preventDefault();
    if (!form.titleRu.trim() || !form.bodyRu.trim()) {
      setErr("Укажите заголовок и текст");
      return;
    }
    setSaving(true);
    setErr("");
    ...
    const payload = {
      ...
      excerptRu: form.excerptRu.trim() || undefined,
    ...
    setMsg(editingId ? "Статья обновлена" : "Статья добавлена");
    setEditorOpen(false);
    load();
  }
  ...
  return (
    <AdminShell title="Статьи" tab="articles" gate={gate}>
      {!gate.unlocked ? null : (
        <>
          <div className="mb-4 flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
            <p className="text-sm text-slate-500">
              Публикации на /ru/articles. Сейчас:{" "}
            ...
            <div className="overflow-hidden rounded-2xl border border-slate-200 bg-white shadow-sm">
              <table className="w-full min-w-[640px] text-left text-sm">
                <thead className="border-b bg-slate-50 text-[11px] uppercase text-slate-400">
                  <tr>
                    <th className="px-3 py-2.5 font-bold">Статья</th>
                  ...
                  {rows.map((r) => (
                    <tr key={r.id} className="border-b border-slate-50">
                      ...
                      <td className="px-3 py-2.5">
                        ...
                        >
                          {r.published ? "Опубликована" : "Скрыта"}
                        </span>
                      </td>
                      <td className="px-3 py-2.5">
              ...
              <div className="relative z-10 flex max-h-[94vh] w-full max-w-2xl flex-col overflow-hidden rounded-t-2xl bg-white shadow-2xl sm:rounded-2xl">
                ...
                >
                  ...
                  <TextArea
                    ...
                    onChange={(v) => setForm((f) => ({ ...f, excerptRu: v }))}
                    rows={2}
                  />
                  <TextArea
                    label="Текст (RU) *"
